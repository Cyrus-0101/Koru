...
/**
 * @file vulkan_utils.c
 * @brief Implementation of utility functions for working with VkResult return types.
 *
 * Provides:
 * - `vulkan_result_string(...)` for readable debug output
 * - `vulkan_result_is_success(...)` for checking if a result should be treated as success
 *
 * These utilities help ensure consistent error handling across all Vulkan API calls.
 */

const char* vulkan_result_string(VkResult result, b8 get_extended) {
    // From: https://registry.khronos.org/vulkan/specs/latest/man/html/VkResult.html
    // Success Codes
    switch (result) {
        default:
        case VK_SUCCESS:
